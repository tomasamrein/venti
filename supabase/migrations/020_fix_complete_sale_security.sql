-- Fix: change complete_sale to SECURITY DEFINER so cashiers can decrement stock.
-- The products_write RLS policy requires owner/admin, blocking cashiers from the
-- stock UPDATE inside complete_sale (SECURITY INVOKER).
-- We add an explicit org-membership check at the top to preserve multi-tenant isolation.
...
BEGIN
  -- Verify caller is an active member of the org (replaces SECURITY INVOKER RLS)
