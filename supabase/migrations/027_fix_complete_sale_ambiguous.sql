-- Fix: "column reference sale_number is ambiguous" in complete_sale.
-- RETURNS TABLE declares an output column also named sale_number, which
-- conflicts with the same-named column in the sales table during the MAX() query.
-- Qualify with the table alias to disambiguate.
...
BEGIN
  ...
  -- Compute next sale number — qualify with table alias to avoid ambiguity
  -- with the output column also named sale_number
