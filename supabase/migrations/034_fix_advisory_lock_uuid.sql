-- Fix: advisory lock key derivation fails with "-" is not a valid hexadecimal digit.
-- UUID::text includes dashes (e.g. "abc12345-..."). substr(uuid, 1, 16) may capture
-- a dash, making the hex cast '0x...-..'::bit(64) blow up.
-- Fix: strip dashes before taking the first 16 hex chars.
...
DECLARE
  ...
  v_lock_key       BIGINT;
BEGIN
  -- Advisory lock per org — strip dashes before hex-casting so the bit cast succeeds
  v_lock_key := ('x' || substr(replace(p_org_id::text, '-', ''), 1, 16))::bit(64)::bigint;
