-- Fix race condition en complete_sale.sale_number
-- Antes: MAX(sale_number) + 1 sin lock → dos ventas simultáneas pueden colisionar.
-- Después: SELECT ... FOR UPDATE con advisory lock por organización.
...
DECLARE
  ...
  v_lock_key       BIGINT;
BEGIN
  -- Advisory lock per organization. Two concurrent calls for the same org
  -- block here until the first commits. Other orgs are unaffected.
  v_lock_key := ('x' || substr(p_org_id::text, 1, 16))::bit(64)::bigint;
  ...
  IF p_payment_method = 'current_account' THEN
    IF p_customer_id IS NULL THEN
      RAISE EXCEPTION 'Cuenta corriente requiere un cliente'
        USING ERRCODE = 'P0001';
    END IF;
    SELECT ca.id, ca.balance INTO v_account_id, v_account_balance
      FROM current_accounts ca
      WHERE ca.organization_id = p_org_id AND ca.customer_id = p_customer_id;
  ...
  -- Now safe: advisory lock guarantees only one concurrent caller for this org
  SELECT COALESCE(MAX(s.sale_number), 0) + 1 INTO v_sale_number
    FROM sales s WHERE s.organization_id = p_org_id;
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    IF v_product_id IS NOT NULL THEN
      SELECT p.track_stock, p.allow_negative, p.stock_current
