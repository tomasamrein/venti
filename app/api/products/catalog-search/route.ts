...
// Cache en memoria por query normalizada (vive mientras el server esté caliente).
...
export async function GET(request: Request) {
  ...
  try {
    const url =
      ...
      '&fields=code,product_name,brands,image_small_url&page_size=30&sort_by=popularity_key'
    ...
    // Timeout / red caída → la UI cae al catálogo curado.
    return NextResponse.json({ results: [] })
  }
}
